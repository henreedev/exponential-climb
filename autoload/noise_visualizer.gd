...
var noise_type_to_rects: Dictionary[NoiseType, Array] = { # of ColorRects
	NoiseType.TERRAIN : [],
	NoiseType.TUNNEL : [],
	NoiseType.RARITY : [],
	NoiseType.QUANTITY : [],
...
func clear_rects():
	for type: NoiseType in noise_type_to_rects.keys():
		for rect: ColorRect in noise_type_to_rects[type]:
			rect.queue_free()
		noise_type_to_rects[type].clear()
...
func regenerate_rects_for_room(room: Room):
	...
	clear_rects()
	for x in range(room._x_bounds.x, room._x_bounds.y + 1):
		for y in range(room._y_bounds.x, room._y_bounds.y + 1):
			for type: NoiseType in noise_type_to_rects.keys():
				noise_type_to_rects[type].append(create_rect(type, Vector2i(x,y)))
				
const TILE_SIZE = Vector2(8,8)
func create_rect(type: NoiseType, tile_pos: Vector2i):
